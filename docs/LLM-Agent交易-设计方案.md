...
| **A1** | OKX 风格订单模型 + 账户账本（保证金/杠杆/强平/TP-SL 触发） | 单测：强平价、保证金率、TP/SL 触发、`reduceOnly` 生效 | 中~大 | ⬜ 待做 |
| **A2** | 决策留痕层（记录结构 + JSONL 落盘 + **回放**） | 单测：同种子回放逐位一致；记录字段完整性 | 中 | ⬜ 待做 |
| **A3** | LLM 接入（Agnes）+ prompt 模板 + 解析 + 风控闸门 | 用 **no-internet 回放**跑通一次；解析失败也留痕 | 中 | ⬜ 待做 |
