...
def build() -> str:
    ...
    L.append("⭐⭐ **而这个「能不能持续」现在已经验了**（见 §4.2 / §4.1）：")
    L.append("把 BTC 换成**更早的 100 天**（与原来那次**不共享任何一根 K 线**）"
             "再跑一遍 ⇒ 效应 **同号且量级相近**；")
    L.append("把 4 个独立块（3 标的 + BTC 第二个时段）合并后，用**保守的**"
             "`t(df=3)` 判据，区间**不跨 0 ⇒ 块级显著**。")
    L.append("")
    L.append("")
    ...
    # ---------- 4.3 待跑的持续性检验（如实标注） ----------
    if not e2:
        L.append("### 4.3 ⏳ 持续性检验：**已设计好、但因额度未跑成**")
        L.append("")
        L.append("§0 说唯一没确定的是**「它能不能持续」**。为此设计了最直接的一测：")
        L.append("")
        L.append("- 把 **BTC 换成更早的 100 天**（2026-03-12 → 06-04），")
        L.append("  与原来那次（06-08 → 09-16）**不共享任何一根 K 线**；")
        L.append("- 用**同一套 KPI 阈值**、同样的 250 段 × 两臂（约 4,000 次调用）；")
        L.append("- 然后看两个时段是**同号**（效应持续）还是**异号**（更像噪声）。")
        L.append("")
        L.append("⚠️ **为什么没跑成**：额度是 **7,500 次 / 5 小时滚动**，")
        L.append("而本轮的 SOL 两臂（4,000 次）刚用完窗口，时段2 一起跑就撞上限流"
                 "（实测 **HTTP 429 占 39%**）。")
        L.append("⭐ 守卫照常拒绝产出 eval（跑下去也只会得到不可用的数据）⇒ 主动停掉。")
        L.append("")
        L.append("⇒ 这一步**只差额度**，其余（代码、前提验证、分析脚本）都已就绪；")
        L.append("见 §4 的投影表：**k=4 就能在块级判出显著**。")
        L.append("")
    ...
    L.append("")
    if e2:
        L.append("⭐⭐ **上一条建议的「做 B」已经做完了**（用户开了额度后立刻跑的）：")
        L.append("")
        L.append("- 时段2 两臂各 2,000 次调用，**失败率 0/2000**，阈值与时段1 **逐字相同**；")
        L.append("- **§4.2：两个时段同号**（−3.51bp vs −4.07bp，量级接近）⇒ 效应**持续**；")
        L.append("- **§4.1：k=4，块级区间 [−7.10, −0.19]bp ⇒ 不跨 0 ⇒ 显著**，"
                 "而且用的是**保守判据**（`t(df=3)` 的假阳性率实测 **0.9%**，不是 5%）。")
        L.append("")
        L.append("| 选项 | 代价 | 状态 |")
        L.append("|---|---|---|")
        L.append("| B. 补时段块 | 4,000 次调用 | ✅ **已完成** ⇒ 结论从「同向复现」"
                 "升级为「**块级显著 + 跨时段持续**」|")
        L.append("| **A. 停手** | 0 | ⭐ **现在推荐这个**："
                 "这个问题已经答完了 |")
        L.append("| C. 换问题 | 0 | 下一步该问的不再是「多大」，而是"
                 "**「要不要为这个代价保留 KPI」**——那是个产品决策，"
                 "价签已在 §2 给出 |")
        L.append("")
        L.append("⇒ ⭐⭐ **本条研究线的结论（可以对外说）**：")
        L.append("")
        L.append("> **给模型加 KPI 会让它显著更「在场」（+8~12pp），代价是每段"
                 "（8 小时）收益下降约 3.6bp；这个代价在 3 个标的 + 2 个不重叠时段上"
                 "一致复现，块级检验在保守判据下显著。按年化算是几十个百分点，"
                 "不是「可以忽略」的量级。**")
        L.append("")
        L.append("⚠️ 但仍要连着说：它**不是可交易信号**，也不代表"
                 "「模型变差了」——KPI 的目的本来就是行为，不是收益。")
    else:
        L.append("⏳ 建议 **做 B**（补 1 个不重叠时段块，约 4,000 次调用 / 0.5 个窗口）："
                 "代价有界、结论有质的提升。")
    L.append("")
    ...
    L.append("")
    if e2:
        L.append("- **年化仍是外推**：−3.6bp/段 是 **2 个不重叠 100 天窗口**的估计；")
        L.append("  乘 1,095 得到年化，前提是「效应在**所有**时段都一样」——")
        L.append("  本轮只验了**两个**时段（同号、量级接近）⇒ **比原来可信，但仍不是证明**。")
        L.append("- **块级显著用的是保守判据**：`t(df=3)`（临界 3.18），"
                 "它自己的假阳性率实测 **0.9%**（比名义 5% 更严）⇒ "
                 "「显著」不是靠放宽判据得来的。")
        L.append("- ⚠️ **但符号翻转检验的对称前提仍被 SOL 的偏度（−12.9）破坏** ⇒ "
                 "那个 p=0.0007 只作旁证，不作为主要依据。")
    else:
