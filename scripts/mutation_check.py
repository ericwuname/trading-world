...
MUTATIONS: list[tuple[str, str, list[tuple[str, str, str]]]] = [
    ...
    (
        'M122',
        '合并统计量的**标定判据被放宽**（`|z|>1.96` → `|z|>2.58`，或大效应时反而不检出）'
        '——于是这次「合并后显著」的自检**本身失效**：'
        '假阳性率会从 5% 掉到 1%，看起来"更严谨"，'
        '实则**再也不会因为标定坏了而报警**。'
        '⚠️ 本轮全项目**第一次**出现收益侧"显著"（跨标的合并 z≈−2.7），'
        '而它是靠这道自检才敢报的 ⇒ 判据坏了，"显著"就没人守了。'
        '⭐ 判据：**凡是用来"certify 结论"的检查，它自己必须被测**——'
        '（假阳性率要能涨、也要能跌；只测一边等于没测）',
        [
            (
                'scripts/a11_cross_asset.py',
                '        if abs(z) > 1.96:\n',
                '        if abs(z) > 2.58:\n',
            ),
        ],
    ...
    (
        'M123',
        '**小 k 的合并区间用正态 1.96 而不是 `t(df=k−1)`** —— '
        'k=3 时正确临界是 **4.30**。用 1.96 会把**本不显著**的结果报成显著：'
        '实测 −3.5bp 的合并效应用正态是 [−5.94, −1.08]bp（不含 0），'
        '用 `t(df=2)` 是 [−8.85, +1.83]bp（**跨 0**）。'
        '⚠️ 而且这不是纯理论担忧：同一个 z 在 δ=0 时的假阳性率实测 **9.9%**'
        '（A11 §3.2 的标定自检）——**反保守是量出来的，不是猜的**。'
        '⭐ 判据：**合并的"单位"是 k 个组均值 ⇒ 自由度是 k−1**，'
        '别把段级的 n 当自由度；**k 越小越要小心**',
        [
            (
                'scripts/a11_cross_asset.py',
                '    tcrit = t_crit95(df) if df >= 1 else float("nan")',
                '    tcrit = 1.96 if df >= 1 else float("nan")',
            ),
        ],
    ),
]
